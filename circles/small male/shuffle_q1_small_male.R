...
sim_results <- data.frame("number of circles" = c(sim_1(small_male))) 

library(writexl)
write_xlsx(sim_results,"C://Users//yuval//OneDrive//english folder//Seminar - clicks//datasets created by simulations//circles//sim_1_shuffle_small_male_q1.xlsx")


# simulation data set 
data_q1_small_male <- read.csv("C://Users//yuval//OneDrive//english folder//Seminar - clicks//datasets created by simulations//circles//sim_1_shuffle_small_male_q1.csv")


# histogram
p <- ggplot(data_q1_small_male, 
            aes(x=ï..number.of.circles,
                fill= factor(ifelse(ï..number.of.circles=="1","Highlighted","Normal"))
            )
) + 
  scale_fill_manual(name = "1", 
  ...
  coord_cartesian(ylim = c(0,0.9), xlim = c(-0.1, 3)) +
  ggtitle("Frequency of number of circles in the shuffle") +
  theme(legend.position = "none")  
p 

# pvalue, 1 - number of circles in the real graph
pvalue(data_q1_small_male,1)

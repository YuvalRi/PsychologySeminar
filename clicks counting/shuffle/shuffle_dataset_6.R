...
pv_right_tail(mean_prop_vec, 0.547)
...
hist_1 <- ggplot(data_for_hists,
  ...
  geom_vline(aes(xintercept = 0.547, size = 0.3),
             color = "#c72727",
             linetype = "dashed",
             size = 0.8) +
  annotate("text",
...
pv_right_tail(mean_prop_vec, 1.714)

# Analysis 2 - Histogram
hist_2 <- ggplot(data_for_hists,
  ...
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  geom_vline(aes(xintercept =  1.714, size = 0.3),
             color = "#c72727",
             linetype = "dashed",
             size = 0.8) +
  annotate("text",
          ...
          size = 5) +
  labs(x = "", y = "Frequency", size = 34, family = "Helvetica") +
  coord_cartesian(ylim = c(0, 0.5), xlim = c(-0.2, 3)) +
  theme(legend.position = "none",
        axis.text.x = element_text(size = 15),
        axis.text.y = element_text(size = 15))
hist_2
...
B <- 10000
diff_mat <- list()
sim_1_diff_female <- function(data, n){
  ...
  sim_1_diff_results_female <- as.data.frame(do.call(rbind, diff_mat))
  return(sim_1_diff_results_female)
}

sim_res_1_female <- sim_1_diff_female(Clicks_sorted_women, 10)

# saving simulation results in excel file
library(rio)
export(sim_res_1_female, "sim_1_diff_q1_female.xlsx")


# the differences in our graph
Clicks_sorted_women <- name_to_number(Clicks_sorted_women)
Clicks_sorted_women <- directed_to_undirected_q3(Clicks_sorted_women, TRUE)
...
diff_data_q1_female <- read_csv("C://Users//yuval//OneDrive//english folder//Seminar - clicks//datasets created by simulations//diff//sim_1_diff_q1_female.csv")

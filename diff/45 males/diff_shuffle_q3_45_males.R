...
sim_res_3 <- sim_3_diff(males_45, 10)

# saving simulation results in excel file
library(rio)
export(sim_res_3, "sim_3_diff_q3_45_males.xlsx")

# the differences in our graph
males_45 <- read.csv("C://Users//yuval//OneDrive//english folder//Seminar - clicks//more datasets//45males_subset.csv")
males_45 <- name_to_number(males_45)
males_45 <- directed_to_undirected_q3(males_45, version = FALSE)
diff_rates(males_45, 10)
mean(as.numeric(diff_rates(males_45, 10))) #2.6

# diff data

diff_data_q3_45_males <- read_csv("C://Users//yuval//OneDrive//english folder//Seminar - clicks//datasets created by simulations//diff//sim_3_diff_q3_45_males.csv")

# adding column - mean of the differences of each row

diff_data_q3_45_males$mean <- rowMeans(diff_data_q3_45_males, na.rm=TRUE)

# histogram
diff_hist_q3_female <- ggplot(diff_data_q3_45_males, aes(x=mean)) + 
  ...
  scale_x_continuous(breaks = seq(1,4.8,0.2)) +
  geom_text(x=2.7, y=0.075, label="2.6") +
  ggtitle("Frequency of differences in the shuffle")
diff_hist_q3_female


mean(as.numeric(diff_rates(males_45, 10))) < quantile(diff_data_q3_45_males$mean, 0.05)

...
sim_res_1 <- sim_1_diff(females_45, 10)

# saving simulation results in excel file
library(rio)
export(sim_res_1, "sim_1_diff_q1_45_females.xlsx")
...
mean(as.numeric(diff_rates(females_45, 10))) #4.4

# diff data

diff_data_q1_45_females <- read_csv("C://Users//yuval//OneDrive//english folder//Seminar - clicks//datasets created by simulations//diff//sim_1_diff_q1_45_females.csv")

# adding column - mean of the differences of each row

diff_data_q1_45_females$mean <- rowMeans(diff_data_q1_45_females, na.rm=TRUE)

# histogram
diff_hist_q1_45_females <- ggplot(diff_data_q1_45_females, aes(x=mean)) + 
  ...
  scale_x_continuous(breaks = seq(2.8,6.5,0.3)) +
  geom_text(x=4.5, y=0.13, label="4.4") +  ggtitle("Frequency of differences in the shuffle")
diff_hist_q1_45_females


#pvalue
mean(as.numeric(diff_rates(females_45, 10))) < quantile(diff_data_q1_45_females$mean, 0.05)

...
pvalue <- function(data, real_value){
  p_vec <- ifelse(data[,7] >= real_value, 0, 1)
  return(mean(p_vec))
}
...
sim_res_1 <- sim_1_diff(small_female, 6)

# saving simulation results in excel file
library(rio)
export(sim_res_1, "sim_1_diff_q1_small_females.xlsx")
...
small_female <- arrange(small_female, ï..Subject)
diff_rates(small_female, 6)
mean(as.numeric(diff_rates(small_female, 6)))

# diff data

diff_data_q1_small_females <- read_csv("C://Users//yuval//OneDrive//english folder//Seminar - clicks//datasets created by simulations//diff//sim_1_diff_q1_small_females.csv")

# adding column - mean of the differences of each row

diff_data_q1_small_females$mean <- rowMeans(diff_data_q1_small_females, na.rm=TRUE)

# histogram
diff_hist_q1_small_female <- ggplot(diff_data_q1_small_females,
                                 aes(x= mean)
) +
  ...
  geom_vline(aes(xintercept = mean(as.numeric(diff_rates(small_female, 6)))),
             color="dodgerblue2",
             linetype="dashed",
             size=1) +
  geom_text(x=0.75,
            y=0.4,
            label="0.666") + 
  coord_cartesian(ylim = c(0, 0.45), xlim = c(0, 1.5)) +
  theme(legend.position = "none") +
  ggtitle("Frequency of differences in the shuffle") 
diff_hist_q1_small_female


#pvalue
pvalue(diff_data_q1_small_females, mean(as.numeric(diff_rates(small_female, 6))))

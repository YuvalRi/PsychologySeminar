...
sim_res_2 <- sim_2_diff(small_female, 6)

# saving simulation results in excel file
library(rio)
export(sim_res_2, "sim_2_diff_q2_small_females.xlsx")
...
diff_data_q2_small_females <- read_csv("C://Users//yuval//OneDrive//english folder//Seminar - clicks//datasets created by simulations//diff//sim_2_diff_q2_small_females.csv")

# adding column - mean of the differences of each row

diff_data_q2_small_females$mean <- rowMeans(diff_data_q2_small_females, na.rm=TRUE)

# histogram
diff_hist_q2_small_female <- ggplot(diff_data_q2_small_females,
                                    aes(x= mean)
) +
  ...
  geom_text(x=1.85,
            y=0.4,
            label="1.666") + 
  coord_cartesian(ylim = c(0, 0.45), xlim = c(0, 5)) +
  theme(legend.position = "none") +
  ggtitle("Frequency of differences in the shuffle") 
diff_hist_q2_small_female

#pvalue
pvalue(diff_data_q2_small_females, mean(as.numeric(diff_rates(small_female, 6))))

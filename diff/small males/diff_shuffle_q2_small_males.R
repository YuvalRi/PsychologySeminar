...
sim_res_2 <- sim_2_diff(small_male, 7)

# saving simulation results in excel file
library(rio)
export(sim_res_2, "sim_2_diff_q2_small_males.xlsx")
...
diff_data_q2_small_males <- read_csv("C://Users//yuval//OneDrive//english folder//Seminar - clicks//datasets created by simulations//diff//sim_2_diff_q2_small_males.csv")

# adding column - mean of the differences of each row

diff_data_q2_small_males$mean <- rowMeans(diff_data_q2_small_males, na.rm=TRUE)

# histogram
diff_hist_q2_small_male <- ggplot(diff_data_q2_small_males,
                                  aes(x= mean)
) +
  ...
  geom_text(x=0.47,
            y=0.3,
            label="0.428") + 
  coord_cartesian(ylim = c(0, 0.35), xlim = c(-0.1, 1)) +
  theme(legend.position = "none") +
  ggtitle("Frequency of differences in the shuffle") 
diff_hist_q2_small_male

#pvalue
pvalue(diff_data_q2_small_males, mean(as.numeric(diff_rates(small_male, 7))))
